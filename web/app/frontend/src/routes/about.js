      ...
      <div class="about">
        ...
        {this.props.isZh ?
          <div>
            <p>
              <strong>财神</strong>
              是基于以太坊智能合约技术的应用。智能合约是在区块
              链上面运行的计算机执行程序，可以准确执行命令，并不受
              制于任何权威及平台。
            </p>
            <p>
              <strong>财神</strong>
              智能合约规定领取红包金额只能在指定日期之后进行。
              比如，您将领取日期设定为2020年1月1日，那么，
              接受人只有在2020年1月1日之后才可以领取收到的金额。
            </p>
            <p>
              您所看到的网页是一个分布式应用（英文称dApp），这个应用中
              仅用户界面存在于我们的服务器上。而
              <strong>财神</strong>最核心的
              智能合约则存在于区块链技术上面。这就意味着即使财神的网站或团
              队不复存在，您的红包将依然长存
            </p>
          </div>
            :
        ...
        <p>Janel Ang 
          {this.props.isZh ?
            <a href="http://angjanel.wixsite.com/sunflakes">(作品集)</a>
              :
            <a href="http://angjanel.wixsite.com/sunflakes">(portfolio)</a>
          }
        ...
        {this.props.isZh ?
          <p>
            红包是重要节庆活动时长辈给晚辈赠送的钱物，通常使用红纸包
            裹代表吉祥。更多关于红包的信息可以在 <a 
              href="https://zh.wikipedia.org/wiki/%E7%B4%85%E5%8C%85"
              target="_blank">
              维基百科</a> 找到。
          </p>
          :
        ...
        {this.props.isZh ?
          <div>
            ...
            <p>
              <strong>财神</strong>
              就是这样的一个智能合约。它通过设计计算机程序，规定指定
              接受人只能在指定日期之后才能接受红包金额。
            </p>
          </div>
          :
          <div>
        ...
        {this.props.isZh ?
          <div>
            ...
            <p>
              只有拥有了密码的指定接受人才可以领取红包。红包的金额牢
              牢锁在智能合约之内，只有拥有正确接受密码的人才可以开锁领取。
            </p>
          </div>
          :
          <div>
        ...
        {this.props.isZh ?
          <p>
            <strong>财神</strong>
            是一个基于以太坊区块链的分布式应用（dApp）。只要以太坊存在，您的红
            包就不会丢失。网页存在与否并不影响这一点。
          </p>
            :
          <p>
        ...
        {this.props.isZh ?
          <p>
            对不起，一旦红包送出将无法取回。您可以试着找到地址所有人，然而我
            们无法帮您去做这件事。
          </p>
          :
          <p>
        ...
        {this.props.isZh ?
          <p>
            这将是您自己的决定。然而财神的价值在于它的去中心化，也就是说，
            您送出和接受的红包不受任何权威限制。
            红包的智能合约本身就决定了您无需依赖任何机构或个人。
          </p>
          :
          <p>
        ...
        {this.props.isZh ?
          <p>
            若赠送者使用本页面送出红包，领取时间将会设定为赠送者所在时区的子
            夜0点。比如说，若赠送者时区设定为东八区，那么领取行为将
            在指定日期的东八区0点之后才能进行。
          </p>
          :
          <p>
